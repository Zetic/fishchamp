...
public class MessageResponder : IResponder<IMessageCreate>
{
    ...
    private async Task<string> HandleFishCommand(ulong userId)
    {
        ...
        if (!userProfile.Inventory.ContainsKey(userProfile.EquippedBait) || 
            userProfile.Inventory[userProfile.EquippedBait] <= 0)
        {
            return $"❌ You don't have any {userProfile.EquippedBait} left! Visit the shop (`!shop`) to buy more.";
        }
        ...
        var success = random.NextDouble() < finalChance;
    ...
    private async Task<string> HandleShopCommand(ulong userId)
    {
        ...
        return $"🏪 **Fishing Shop**\n\n" +
               ...
               "🛒 **Available Items:**\n" +
               "• Basic Worms: 2 🪙 each\n" +
               "• Steel Rod: 150 🪙 (Level 2+)\n" +
               "• Pro Rod: 300 🪙 (Level 3+)\n" +
               "• Ice Rod: 450 🪙 (Level 4+)\n" +
               "• Lightning Rod: 600 🪙 (Level 5+)\n" +
               "• Mystic Rod: 750 🪙 (Level 6+)\n\n" +
               "💰 **Sell Fish:**\n" +
               "Use `!sell <fish_name>` or `!sell all` to sell your fish for gold!\n\n" +
               "🛍️ **Buy Items:**\n" +
               "• `!buy worms` - 2 🪙 for 10 worms\n" +
               "• `!buy steel rod` - 150 🪙 (Level 2+)\n" +
               "• `!buy pro rod` - 300 🪙 (Level 3+)\n" +
               "• `!buy ice rod` - 450 🪙 (Level 4+)\n" +
               "• `!buy lightning rod` - 600 🪙 (Level 5+)\n" +
               "• `!buy mystic rod` - 750 🪙 (Level 6+)";
    }
