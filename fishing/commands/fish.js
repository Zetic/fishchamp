...
async function executeMessageCommand(message) {
  try {
    ...
    const currentArea = fishData.areas.find(area => area.name === userProfile.area);
    if (!currentArea) {
      await message.reply({
    ...
    const areaEmbed = new EmbedBuilder()
      ...
      .addFields({ name: 'Fish Available', value: currentArea.fishPool.join(', ') })
      .setColor(0x3498DB);
...
async function executeSlashCommand(interaction) {
  try {
    ...
    // Start fishing directly
    await fishingInteraction.startFishing(interaction);
  } catch (error) {
    console.error('Error handling fish command:', error);
    await interaction.reply({
